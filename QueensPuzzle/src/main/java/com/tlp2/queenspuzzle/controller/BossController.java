...
public class BossController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        tempoRestante = 50 + sessao.getUpgradeAtual().getTempoBonus();

        labelStatus.setText("Tabuleiro 8×8 — Tempo reduzido!");
        desenharTabuleiro();
        iniciarTimer();
    }
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                ...
                StackPane celula = criarCelula(linha, col, tam, n);
                        ...
                        celula.setOnMouseDragEntered(e -> {
                            if (arrastandoMarcacao && !tabuleiro.temRainha(linha, col) && !tabuleiro.temMarcacao(linha, col)) {
            ...
            private StackPane criarCelula(int linha, int col, double tam, int n) {
                ...
                if (tabuleiro.temMarcacao(linha, col)) {
                    Text x = new Text("❌");
                    x.setStyle("-fx-font-size: " + (tam * 0.40) + ";");
            ...
            else if (tabuleiro.temMarcacao(linha, col)) {
    ...
    if (tabuleiro.estaSolucionado()) {
        ...
        Timeline espera = new Timeline(new KeyFrame(Duration.seconds(2), e -> MainApp.trocarTela("/com/tlp2/queenspuzzle/view/Upgrades.fxml")));
